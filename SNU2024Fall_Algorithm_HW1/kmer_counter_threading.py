...
# k=10, Nonpathogenic 으로 했을 때 6.671s (텍스트 파일 삭제)
# k=12, Pathogenic 으로 했을 때, 36.075s 
...
def main():
    ...
    kmer_counts = process_file_multithreaded(k, input_file)
    
    # 결과를 출력 파일에 작성
    write_output(kmer_counts, output_file, k)


def process_file_multithreaded(k, input_file):
    kmer_counts = defaultdict(int)
    lock = threading.Lock()  # kmer_counts에 동시 접근을 방지하기 위한 Lock 객체
    threads = []
    ...
    with open(input_file, 'r') as file:
        ...
        for line in file:
            ...
            if line.startswith('>'):
                if current_sequence:
                    # 각 Chromosome을 개별 스레드로 처리
                    t = threading.Thread(target=count_kmers_in_sequence, args=(current_sequence, k, kmer_counts, encode_array, lock))
                    threads.append(t)
                    t.start()
                    current_sequence = ''
            else:
...
def count_kmers_in_sequence(sequence, k, kmer_counts, encode_array, lock):
