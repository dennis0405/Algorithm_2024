...
# k=12, Pathogenic 으로 했을 때, 42.258s 
...
def main():
    ...
    generate_kmer_cache(k, max_kmer_count)

    # 주어진 k값과 input_file을 바탕으로 kmer을 count (dict 형태)
    kmer_counts = process_file(k, input_file)
    
    # 결과를 출력 파일에 작성
    write_output(kmer_counts, output_file, max_kmer_count)
...
def generate_kmer_cache(k, max_kmer_count):
    for kmer_encoded in range(max_kmer_count):
        kmer = [''] * k
        kmer_encoded_copy = kmer_encoded
        offset = k-1
        for i in range(k):
            kmer[offset-i] = decode_list[kmer_encoded_copy & 0b11]
            kmer_encoded_copy >>= 2
        kmer_cache[kmer_encoded] = ''.join(kmer)

# 캐시 사용
def generate_kmer(kmer_encoded):
    return kmer_cache[kmer_encoded]


def write_output(kmer_counts, output_file, max_kmer_count):
    # 입출력 개선
    buffer = []
